...
;(function($) {
	Util.Dialog = function(o) {
		defaults = $.extend({
			...
			height: "",//窗口高度;
			time: "",//自动关闭等待时间;(单位秒);
			drag: false,//是否启用拖动( 默认为启用);
			lock: true,//是否限制拖动范围；
			fixed: true,//是否开启固定定位;
			showbg: false,//是否显示遮罩层( 默认为false);
			showborder: false,//是否显示透明边框
			...
			cfns: "",//弹出窗关闭后执行的函数;
			ofns: "", //弹出窗打开后执行的函数;
			animateClose:""//关闭前执行的函数  主要用作关闭动画
		}, o);
	...
	$.extend(Util.Dialog, {
		...
		init: function(o) {
			...
			if(typeof o.time === "number") {
				setTimeout(function() {
					Util.Dialog.close(o.boxID, o.cfns,o.animateClose);
				}, o.time);
			};
			...
			$("#BYTipsWindowBg").bind("dblclick", function(e) {
				e = e || window.event;
		...
		create: function(o) {
			var X='';
			if(o.showX){
				X="<span class=\"ui_btn_close\"> x </span>";
			}
			...
			boxDom += "<tr><td><div class=\"ui_title_wrap\"><div class=\"ui_title\"><div class=\"ui_title_text\"><span class=\"ui_title_icon\"></span>" + o.title + "</div><div class=\"ui_btn_wrap\">"+X+" </div></div></div></td></tr>";
		...
		loadContent: function(o) {
			...
			var _width = o.width?o.width:'auto', _height =  o.height;
			$contentID.css({
				width: _width,
				height: _height
			});
			...
			var drag = function() {
					...
					Util.loadJS("BY_Drag.js", function() {
						Util.drag({
							obj: o.boxID,
			...
			if(o.showtitle != true) {$('.ui_button_wrap',Util.Dialog.box).remove();};
			...
			case "img":
				$.ajax({
					...
					success: function(html) {
						...
						Util.loadJS("BY_Img.js", function() {
			...
			case "swf":
				$.ajax({
					...
					success: function(html) {
						Util.loadJS("BY_Swf.js", function() {
			...
			case "iframe":
				...
				$.ajax({
					beforeSend: function() {
						$contentID.addClass("ui_loading").html("<img src='" + Util.config.JSfile + "loading.gif' class='ui_box_loading' />")
						Util.Dialog.setPosition(o);
					},
					error: function() {
						$contentID.removeClass("ui_loading").html("<p class='ui_box_error'><span class='ui_box_callback_error'></span>URL地址错误！</p>");
						Util.Dialog.setPosition(o);
					},
					success: function(html) {
		...
		close: function(obj, cfns,animateClose) {
			...
			if(box.length != 0) {
				...
				if(animateClose != "" && $.isFunction(animateClose)) {
					//需要在回调中执行    Util.Dialog.close('dialog-login');   以关闭窗口
					animateClose();
				...
				if(!box.is(":animated")){
					...
					Util.clearIframe();
					box.parent().remove();
					$("#BYTipsWindowBg").animate({
						opacity: "0"
					}, 100, function() {
						$(this).remove();
					});
					for(var i = 0; i < Util.Dialog.data.winarr.length; i++) {
						if(obj == Util.Dialog.data.winarr[i][0]){
							Util.Dialog.data.winarr.remove(Util.Dialog.data.winarr[i]);
						} 
					};
